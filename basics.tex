...
The discussion above shows, that we can not establish the supermartingale property for $S_n$ without further restrictions, by the same arguments as were presented in \cite{bose1999strong} and \cite{dikta2000strong}. The following Lemma will establish the supermartingale property for $S_n$ under the additional assumption, that $q$ is non-decreasing. 
\begin{lemma} \label{lem:sn_supermart}
	Let $q(z)$ be non-decreasing for all $z\in \R_+$. Then $S_n(q)$ is a non-negative reverse supermartingale.
	%
	\begin{proof}
		First note that 
		\begin{align*}
		...
		%
		Next consider that we have 
		$$q_i-q_{i+1} \leq 0 \quad\textrm{ and }\quad q_{i+1}(1-q_i) \geq q_{i+1} - q_i \geq 0\mcomma$$ 
		since $q(z)$ is non-decreasing in $z$. Now combining the latter with equation \eqref{eq:qip1_qi} yields
		\begin{align*}
...
%\end{remark}
%
The assumption $q$ is monotone non-decreasing in Lemma \ref{lem:sn_supermart} is transfered to the censoring model $m$ by (A\ref{ass:m_increas}). This poses a restriction on the models $m$ which can be used. This will be discussed in more detail in Chapter \ref{ch:model}.
%
%
%
...
\begin{thm}\label{thm:ex_limit_sn}
	Let $q(z)$ be non-decreasing for all $z\in \R_+$. Then $S_\infty = \lim_{n\to\infty} S_n(q)$ exists almost surely and 
